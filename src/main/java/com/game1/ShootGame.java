...
public class ShootGame extends JPanel {
	...
	private FlyingObject[] flyings = {}; // ?§Ý?????
	...
	public static void main(String[] args) {
		...
		frame.setLocationRelativeTo(null); // ?????????¦Ë??
	...
	public void bang(Bullet bullet) {
		...
		for (int i = 0; i < flyings.length; i++) {
			FlyingObject obj = flyings[i];
			if (obj.shootBy(bullet)) { // ?§Ø???????
				index = i; // ????????§Ö???????????
				break;
			}
		}
		if (index != -1) { // ?§Ý??§Ö??????
			FlyingObject one = flyings[index]; // ????????§Ö??????

			FlyingObject temp = flyings[index]; // ?????§Ö??????????????????????
			flyings[index] = flyings[flyings.length - 1];
			flyings[flyings.length - 1] = temp;

			flyings = Arrays.copyOf(flyings, flyings.length - 1); // ???????????????(???????§Ö?)
